...
CHART_DIR="${ROOT_DIR}/charts/external-service-discovery-operator"
...
mkdir -p "${OUTPUT_DIR}"
rm -f \
  "${OUTPUT_DIR}"/external-service-discovery-operator-*.tgz \
  "${OUTPUT_DIR}"/external-service-discovery-operator-*.tgz.prov

helm lint "${CHART_DIR}"

helm template external-service-discovery-operator "${CHART_DIR}" \
